...
# Build the solution
RUN dotnet build Participants.Api/Participants.Api.csproj -c Release -o /home/app/build

# Publish the solution
RUN dotnet publish Participants.Api/Participants.Api.csproj -c Release -o /home/app/publish

# Final stage to run the solution
FROM base AS final
WORKDIR /home/app
COPY --from=buildbase /home/app/publish .
ENTRYPOINT ["dotnet", "Participants.Api.dll"]
