...
# Build the solution
RUN dotnet build Hunts.Api/Hunts.Api.csproj -c Release -o /home/app/build

# Publish the solution
RUN dotnet publish Hunts.Api/Hunts.Api.csproj -c Release -o /home/app/publish

# Final stage to run the solution
FROM base AS final
WORKDIR /home/app
COPY --from=buildbase /home/app/publish .
ENTRYPOINT ["dotnet", "Hunts.Api.dll"]
